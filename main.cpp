...
void ReadFile(const char* filename, map<string, Color>& colors)
{
   // TODO: Read the file, create and store some Color objects
	ifstream colorFile;
	colorFile.open(filename);
	string colorName;
	string colorValueStr;
	int colorValueInt;
	while (!colorFile.eof()) {
		getline(colorFile, colorName, ' ');
		getline(colorFile, colorValueStr);
		Color tempColor;
		colorValueInt = stoi(colorValueStr);
		tempColor.SetValue(colorValueInt);
		tempColor.SetName(colorName.c_str());
		colors.emplace(colorName, tempColor);
	}
}
void PrintColors(const map<string, Color>& colors)
{
...
void SearchForColor(map<string, Color>& colors)
{
	...
	string searchTerm;
	cin >> searchTerm;
	auto it = colors.find(searchTerm);
	if (it == colors.end()) {
